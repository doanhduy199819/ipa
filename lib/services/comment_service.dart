...
mixin CommentService {
  ...
  Future<void> addCommentToExperiencePost(String content, String experiencePostId){
    Comment comment = Comment.createdNowFromCurrentUser(content);
    DocumentReference docRef =
    _db.collection('experience').doc(experiencePostId).collection('comments').doc();
    comment.id ??= docRef.id;
    comment.author_id ??= AuthService().currentUserId;
    return docRef.set(comment.toJson()).then((_) {
      debugPrint(
          'New comment is added to Experience Post $experiencePostId\n with id ${docRef.id}');
    });
  }
  ...
  Stream<List<Comment>?> commentsFromExperiencePost(String experiencePostId){
    return _db
  ...
  Future<void> deleteCommentFromExperiencePost(
      String commentId, String articleId) async {
    _db
        .collection('experience')
        .doc(articleId)
  ...
  Stream<int> getLikeOfEachCommentInExperiencePost(String experiencePostId, String commentId) {
    return _db
        .collection('experience')
        .doc(experiencePostId)
        .collection('comments')
  ...
  Future<bool> isLikedCommentInExperiencePost(String experiencePostId, String commentId) async{
    if (AuthService().currentUserId == null) {
      throw Exception('No user');
    }
    bool res = false;
    ...
    await _db
      ...
      final data = docSnap.data();
      print('LikeUsers has data comment');
      List<String>? list =
      data?['upvote_users'] is Iterable
          ? List.from(data?['upvote_users'])
          : null;
      res = list?.contains(userId) ?? false;
    });
  ...
  void likeCommentInExperiencePost(String experiencePostId, String commentId,bool active){
    if (active) {
      _db.collection('experience').doc(experiencePostId).collection('comments').doc(commentId).update({
        "upvote_users": FieldValue.arrayUnion([AuthService().currentUserId]),
      });
      debugPrint('Upvote success');
    } else {
